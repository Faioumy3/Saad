...
export const Select: React.FC<React.SelectHTMLAttributes<HTMLSelectElement> & { label: string }> = ({ label, children, ...props }) => (
  <div className="mb-4">
    <label className="block text-gray-700 font-medium mb-2 text-right">{label}</label>
    <select 
      className="w-full p-3 border-2 border-gray-200 rounded-lg focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all text-right bg-white"
      {...props}
    >
      {children}
    </select>
...
export const StatCard: React.FC<{ label: string, value: string | number, colorClass?: string, icon?: LucideIcon }> = ({ 
  label, value, colorClass = "text-primary", icon: Icon 
}) => (
